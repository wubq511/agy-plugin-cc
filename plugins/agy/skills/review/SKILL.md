...
3. The tool returns the diff, git context, and review instructions. **You (Claude Code) execute the review** — apply the review criteria to the diff and produce a structured JSON result.
...
5. If issues are found, suggest next steps:
   - Delegate fixes back to Antigravity: `/agy:delegate`
   - Fix directly in Claude Code
...
- "Review the Antigravity changes" → standard review
- "Adversarial review of what Antigravity did" → adversarial review
- "Review focusing on security" → `agy_review` with `focus="security"`
- "Review job agy-abc" → `agy_review` with `job="agy-abc"` (prefix matching)
