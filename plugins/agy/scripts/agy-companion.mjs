...
/**
 * Antigravity Companion — MCP Server for Claude Code
 *
 * Implements the MCP stdio protocol (2025-03-26) with 6 tools:
 * - agy_delegate:    Delegate a coding task to Antigravity CLI
 ...
 *
 * Symmetric counterpart to cc-plugin-codex's cc-companion.mjs.
 * Direction: Claude Code → Antigravity CLI (not Codex → Claude Code).
 *
 * Key differences from cc-companion.mjs:
 * - Tool prefix: agy_ (not cc_)
 * - Job ID prefix: agy- (not cc-)
 * - State dir: agy-companion (not cc-companion)
 * - CLI binary: agy (not claude)
 * - Resume flags: --continue/--conversation (not --resume-last/--resume)
 * - Timeout: --print-timeout (not spawnSync timeout)
 * - No --effort (embedded in model name)
 * - No --allowedTools (prompt injection for write=false)
 * - Dynamic models via `agy models` (not hardcoded)
 * - JSON mapping: response→result, conversation_id→sessionId, duration_seconds→duration, no cost
...
import { runAntigravitySync, runAntigravityDetached, getAntigravityAvailability, getAvailableModels } from "./lib/antigravity-runner.mjs";
...
const TOOLS = [
  {
    name: "agy_delegate",
    description: "Delegate a coding task to Antigravity CLI. Antigravity executes in a separate process and returns results automatically. By default runs in foreground (waits for completion). Set background=true to return immediately with a job ID. Use resume=true to continue the last Antigravity session, or resumeSession=<id> to resume a specific session.",
    inputSchema: {
      type: "object",
      properties: {
        task: { type: "string", description: "The coding task to delegate to Antigravity CLI" },
        write: { type: "boolean", description: "Allow Antigravity to write files (default: true). When false, injects read-only prompt and omits --dangerously-skip-permissions as fallback." },
        background: { type: "boolean", description: "Run in background, return job ID immediately (default: false)" },
        model: { type: "string", description: "Antigravity model name (free string, passed to --model). Use agy_list_models to see available options." },
        dangerouslySkipPermissions: { type: "boolean", description: "Skip permission prompts (default: false, set true to allow Antigravity to write without confirmation)" },
        resume: { type: "boolean", description: "Resume the last Antigravity session (adds --continue flag)" },
        resumeSession: { type: "string", description: "Resume a specific Antigravity session by conversation ID (adds --conversation <id> flag)" },
        timeout: { type: "string", description: "Timeout duration for the task (e.g., '5m0s', '10m0s'), mapped to --print-timeout (default: 5m0s)" }
      },
      required: ["task"]
    }
  },
  {
    ...
    description: "List available Antigravity models dynamically by running `agy models`. Models change over time — never hardcode. Effort is embedded in model names (e.g., 'Gemini 3.5 Flash (Low)').",
  ...
  {
    name: "agy_cancel",
    description: "Cancel a running Antigravity job. Without arguments, cancels the latest active job. Accepts job ID prefix.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  {
    name: "agy_review",
    description: "Review code changes made by an Antigravity job. Returns the diff and a structured review prompt for Claude Code to execute. Set adversarial=true to challenge implementation choices with a structured XML template. Auto-detects review scope (working-tree vs branch). Returns findings in a structured schema with verdict, severity-ranked findings, and next steps.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  {
    name: "agy_setup",
    description: "Check if Antigravity CLI is installed and ready to use. Reports version, availability, and any issues.",
    inputSchema: { type: "object", properties: {} }
  }
];
...
function handleDelegate(params) {
  ...
  const jobId = generateJobId("agy");
  const now = new Date().toISOString();
  const taskTitle = resume ? "Antigravity Resume" : "Antigravity Task";
  ...
  if (background) {
    ...
    const child = runAntigravityDetached(task, {
      cwd: workspaceRoot, write, model,
      dangerouslySkipPermissions: skipPerms, resume, resumeSession, timeout,
      resultFile
    });
    ...
    child.on("error", (err) => {
      ...
      updateJob(workspaceRoot, {
        ...
        errorMessage: `Failed to start agy: ${err.message}`
      });
  ...
  // Foreground mode
  appendLogLine(workspaceRoot, jobId, "Running agy -p synchronously.");
  updateJob(workspaceRoot, { id: jobId, phase: "executing" });

  const result = runAntigravitySync(task, {
    cwd: workspaceRoot, write, model,
    dangerouslySkipPermissions: skipPerms, resume, resumeSession, timeout
  });
  ...
  if (result.ok) {
    ...
    appendLogLine(workspaceRoot, jobId, `Done. Duration: ${formatDuration(result.duration)}.`);
...
function handleListModels() {
  ...
  const workspaceRoot = resolveWorkspaceRoot(cwd);
  const result = getAvailableModels(workspaceRoot);
  ...
  const table = [
    "| # | Model Name |",
    "|---|------------|",
    ...result.models.map((m, i) => `| ${i + 1} | ${m} |`)
  ].join("\n");

  const guide = [
    "### Selection Guide",
    "- Effort is embedded in the model name (e.g., \"(Low)\", \"(Medium)\", \"(High)\")",
    "- **Simple tasks**: Choose a model with (Low) effort",
    "- **Feature implementation**: Choose a model with (Medium) effort",
    "- **Complex refactor**: Choose a model with (High) effort",
    "- Pass the full model name string to `agy_delegate`'s `model` parameter"
  ].join("\n");
  ...
  if (params.all === true) {
    ...
    const table = [
      ...
      ...sorted.map((j) => {
        ...
        return `| ${j.id} | ${j.status} | ${j.phase || "—"} | ${taskShort} | ${j.model || "—"} | ${formatDuration(j.duration)} |`;
      })
    ].join("\n");
...
function buildStandardReviewPrompt(focus) {
  return `You are reviewing code changes made by Antigravity. Produce a structured review.
...
function handleReview(params) {
  ...
  // Schema reference matching cc-plugin-codex format
...
function handleSetup() {
  ...
  const lines = ["## Antigravity Companion Setup\n"];

  if (agyStatus.available) {
    lines.push(`✅ Antigravity CLI: ${agyStatus.detail}`);
  } else {
    lines.push(`❌ Antigravity CLI: ${agyStatus.detail}`);
  }
  ...
  if (agyStatus.available && nodeStatus.available) {
    ...
    lines.push("\n❌ Setup incomplete");
    if (!agyStatus.available) {
      lines.push("Install Antigravity CLI: download binary from official source, then run `agy install`");
    }
    if (!nodeStatus.available) {
...
const HANDLERS = {
  agy_delegate: handleDelegate,
  agy_list_models: handleListModels,
  agy_check: handleCheck,
  agy_cancel: handleCancel,
  agy_review: handleReview,
  agy_setup: handleSetup
};
    ...
    switch (msg.method) {
      case "initialize": {
        sendResponse(msg.id, {
          ...
          serverInfo: { name: "agy-companion", version: "0.1.0" },
          instructions: "Antigravity Companion: delegate coding tasks to Antigravity CLI from Claude Code. Supports job logs, phase tracking, adversarial review, session scoping, prefix matching, and resume."
        });
        break;
      }
      case "tools/list": {
